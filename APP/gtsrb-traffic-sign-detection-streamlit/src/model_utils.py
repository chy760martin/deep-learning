...
class TransferLearningModel(nn.Module):
    def __init__(self, base_model, feature_extractor, num_classes=43):
        super().__init__()
        ...
        base_model.fc = nn.Sequential(
            ...
            nn.ReLU(), # 활성화 함수
            # nn.BatchNorm1d(256),
            nn.LayerNorm(256), # LayerNorm을 사용하면 배치 크기 1에서도 안전하게 학습
            nn.Dropout(p=0.3), # 드롭아웃

            nn.Linear(256, 64), # 은닉층
            nn.ReLU(),
            # nn.BatchNorm1d(64),
            nn.LayerNorm(64), # LayerNorm을 사용하면 배치 크기 1에서도 안전하게 학습
            nn.Dropout(p=0.3),
