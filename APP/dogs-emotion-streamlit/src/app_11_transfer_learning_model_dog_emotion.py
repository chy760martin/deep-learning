# app_11_transfer_learning_model_dog_emotion.py
# Streamlit 앱 - 강아지 감정 분류기
...
def load_model():
    ...
    num_classes=4
    model = TransferLearningModel(base_model, feature_extractor=True, num_classes=num_classes).to(device) # - feature_extractor=True: 특징 추출기로 사용, num_classes=4: 강아지 4 클래스

    model_path = os.path.join("models", "model_transfer_learning_dog_emotion.pt") # - 학습된 모델 가중치 로드
...
# Streamlit UI
st.title("강아지 감정 분류기") # - 앱 제목
st.write("이미지를 업로드하면 AI가 강아지 감정을 예측해줍니다!") # - 앱 설명
